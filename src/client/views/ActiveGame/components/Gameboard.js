...
import { isInCellList, isInvalidEntry } from "../../../utils";
...
const Gameboard = ({
  ...
  const handleMouseOver = (e, cell) => {
    ...
    if (previousCell && previousCell.id === cell.id) {
      ...
      const nextInput = nextHighlightedCells
        .map(({ letter }) => letter)
        .join("");
      setInput(nextInput);
  ...
  const formatLetter = (letter) => (letter === "QU" ? "Qu" : letter);

  // console.warn({ gameboardData });

  const boardDimension = gameboardData.dimension;
        ...
        {gameboardData.board.map((cell) => (
          <Cell
            ...
            className={classNames("cell", {
              highlighted: isInCellList(highlightedCells, cell),
            })}
