...
const getLettersForBoard = (frequencyTable, boardDimension) => {
  ...
  for (let i = 1; i <= numberOfSquaresOnBoard; i++) {
    ...
    for (let j = 0; j < ALPHABET.length; j++) {
      ...
      if (
        currentLetter === "A" &&
        randomNumberWithinRange < currentLetterRange
      ) {
        letters.push(currentLetter);
        break;
      }
      if (
        currentLetterRange <= randomNumberWithinRange &&
        nextLetterRange > randomNumberWithinRange
      ) {
...
export const createGameBoard = (options) => {
  ...
  return { board, words, dimension: boardDimension };
};
