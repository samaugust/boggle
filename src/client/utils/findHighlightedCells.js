...
const getMatchesFromCell = (
  ...
  if ((currentLetter === currentCell.letter && input.length === 1)
    || (currentLetter === currentCell.letter && currentLetter === 'QU' && input.length === 2))
    return [...potentialMatch, currentCell]

  if (currentLetter !== currentCell.letter) return []

  const uncheckedAdjacentCells = getAdjacentCells(currentCell, board, alreadyChecked)
